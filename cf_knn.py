...
def recommend(target_user_id, n_recommendations=10, num_neighbors=20, print_output=True):
    ...
    knn = NearestNeighbors(num_neighbors, metric='cosine').fit(user_movie_df)
    _, indices = knn.kneighbors(user_movie_df, num_neighbors)
    ...
    knn_matrix = user_movie_df.iloc[nbr_indices,]
    ...
    user_ratings = user_movie_df.iloc[user_index]
